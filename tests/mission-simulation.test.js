...
test('схематическая орбита остаётся выше поверхности Земли', () => {
    readyIds.forEach(id => {
        const mission = missions.find(item => item.id === id);
        for (let step = 0; step <= 100; step += 1) {
            const position = simulation.positionAtProgress(mission, step / 100);
            assert.ok(position.radiusKm > simulation.EARTH_RADIUS_KM, `${id}: ${step}`);
        }
    });
});
...
test('ключевые события появляются в заданной точке сценария', () => {
    ...
    assert.equal(simulation.currentEventAtProgress(sputnik, 0.9).id, 'launch');
    assert.equal(simulation.currentEventAtProgress(sputnik, 1).id, 'mission-end');
    assert.equal(simulation.currentEventAtProgress(voskhod, 0.2).id, 'first-eva');
});
